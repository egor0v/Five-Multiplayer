	...
	public:
		...
		void EnsureInit()
		{
			...
			*/
			...
			HMODULE dllHandler = GetModuleHandle(0);

			if (dllHandler == NULL)
				return;

			m_begin = reinterpret_cast<uintptr_t>(dllHandler);
			auto dosHeader = reinterpret_cast<PIMAGE_DOS_HEADER>(dllHandler);

			auto ntHeader = reinterpret_cast<PIMAGE_NT_HEADERS>(m_begin + dosHeader->e_lfanew);
			/*
			m_begin = reinterpret_cast<uintptr_t>(getRVA<void>(0));
			auto dosHeader = getRVA<IMAGE_DOS_HEADER>(0);
			auto ntHeader = getRVA<_IMAGE_NT_HEADERS64>(dosHeader->e_lfanew);
			*/
			m_end = m_begin + ntHeader->OptionalHeader.SizeOfCode;
