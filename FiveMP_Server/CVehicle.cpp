...
void CVehicle::Reset()
{
	...
	memset(&m_matWorld,0,sizeof(MATRIX2X4));
...
void CVehicle::Update(EntityId playerID, MATRIX2X4 * matWorld, Vector3 * vecMoveSpeed, Vector3 * vecTurnSpeed, float fHealth)
{
	m_driverID = playerID;
	memcpy(&m_matWorld, matWorld, sizeof(MATRIX2X4));
...
{
	if (m_driverID != INVALID_ID) { //Fixed the retarded bug
