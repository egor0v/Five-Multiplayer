...
void CPlayer::BroadcastSyncData()
{
	...
	if(m_byteUpdateFromNetwork == UPDATE_TYPE_FULL_ONFOOT)
	{
		...
		CPlayer * pPlayer = pNetowkManager->GetPlayerManager()->GetAt(m_bytePlayerID);
		if (pPlayer) {
			memcpy(&playerSyncData.vecPos, &m_vecPos, sizeof(Vector3));
			//pPlayer->SetQuaternion(playerSyncData.matrix.quat);
	...
	else if(m_byteUpdateFromNetwork == UPDATE_TYPE_FULL_INCAR)
	{
		...
		CVehicle * pVehicle = pNetowkManager->GetVehicleManager()->GetAt(vehicleSyncData.vehicleID);
		if (pVehicle) {
			pVehicle->SetQuaternion(vehicleSyncData.matrix.quat);
			//memcpy(&vehicleSyncData.vecDirection, &m_vecDirection, sizeof(float));
...
void CPlayer::StoreOnFootFullSyncData(PLAYER_SYNC_DATA * pPlayerSyncData)
{
	...
	//printf("Received onFootFullSyncData Net Position: %f %f %f Server Position %f %f %f", pPlayerSyncData->vecPos.X, pPlayerSyncData->vecPos.Y, pPlayerSyncData->vecPos.Z, m_vecPos.X, m_vecPos.Y, m_vecPos.Z);
	...
	memcpy(&m_speed, &pPlayerSyncData->m_speed, sizeof(Vector3));
...
void CPlayer::StoreAimSyncData(Vector3 * pAim)
{
	m_bHasAim = true;
	memcpy(&m_vecAimPos, pAim, sizeof(Vector3));
}
...
void CPlayer::StoreInCarFullSyncData(VEHICLE_SYNC_DATA * pVehicleSyncData)
{
	...
	*/
	...
	//memcpy(&m_vecRoll, &pVehicleSyncData->vecRoll, sizeof(float[4*4]));
	//std::copy(pVehicleSyncData->vecRoll, pVehicleSyncData->vecRoll + 16, m_vecRoll);
	/*
	for (int x = 0; x<4 * 4; x++) {
		m_vecRoll[x] = pVehicleSyncData->vecRoll[x];
	}
	*/
	pVehicle->SetQuaternion(pVehicleSyncData->matrix.quat);
	//memcpy(&m_vecDirection, &pVehicleSyncData->vecDirection, sizeof(float)); //We don't need this anymore, old vice city only
	...
	//memcpy(&matWorld.vecRoll, &pVehicleSyncData->vecRoll, sizeof(float[4*4]));
	//std::copy(pVehicleSyncData->vecRoll, pVehicleSyncData->vecRoll + 16, matWorld.vecRoll);
	/*
	for (int x = 0; x<4 * 4; x++) {
		m_vecRoll[x] = pVehicleSyncData->vecRoll[x];
	}
	*/
	//memcpy(&matWorld.vLookUp, &pVehicleSyncData->vecDirection, sizeof(float));
	...
	//std::copy(pVehicleSyncData->vecPos, pVehicleSyncData->vecPos + 3, matWorld.vPos);
