...
void CVehicleManager::Process()
{
	for(BYTE x = 0; x < MAX_VEHICLES; x++) {
		if(m_pVehicles[x]) {
			...
			//Great, so technically the car has been driven every single time!!! no way of knowing!!! amazing code!!! please go work at google now!!
			...
			if(m_pVehicles[x]->HasBeenDriven()) {
				...
				if((GetTickCount() - m_pVehicles[x]->GetTimeSinceLastDriven()) > 10000) { //250000
					// respawn this vehicle
					m_pVehicles[x]->Respawn();
					pScripts->onVehicleSpawn(x);
				}
