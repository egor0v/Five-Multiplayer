...
#endif
	if (PLAYER::GET_PLAYER_PED(0))
		curPedPtr = (Ped*)PLAYER::GET_PLAYER_PED(0);
...
void CPlayerPed::Create(int iSkin, float fX, float fY, float fZ, float fRotation)
{
	if (iSkin > (sizeof(pedModels) / sizeof(*pedModels)) || iSkin < 0)
		iSkin = 0;

	if (strlen(pedModels[iSkin]) < 1) iSkin = 0;
	int PedHash = GAMEPLAY::GET_HASH_KEY(pedModels[iSkin]);
	...
	curPedPtr = (Ped*)PED::CREATE_PED(21, PedHash, fX, fY, fZ, fRotation, 1, true);
	STREAMING::SET_MODEL_AS_NO_LONGER_NEEDED(PedHash);
	//	}
	//}
	//Set the relationships to the world
	PED::ADD_RELATIONSHIP_GROUP("SYNCPED", &relGroup);
	PED::SET_RELATIONSHIP_BETWEEN_GROUPS(3, (Hash)relGroup, PED::GET_PED_RELATIONSHIP_GROUP_HASH((Ped)&curPedPtr));
	PED::SET_RELATIONSHIP_BETWEEN_GROUPS(3, PED::GET_PED_RELATIONSHIP_GROUP_HASH((Ped)&curPedPtr), (Hash)relGroup);

	//Prevent peds from fleeing and doing random crap we don't wanna see that could mess up the sync
	PED::SET_BLOCKING_OF_NON_TEMPORARY_EVENTS((Ped)curPedPtr, TRUE); //the most important one
	PED::SET_PED_FLEE_ATTRIBUTES((Ped)curPedPtr, 0, 0);
	PED::SET_PED_COMBAT_ATTRIBUTES((Ped)curPedPtr, 46, TRUE);

	//Disable ragdolls and set as invincible (We'll inflict damage ourselves somehow by sending calls to the clients)
	ENTITY::SET_ENTITY_INVINCIBLE((Entity)curPedPtr, true);
	PED::SET_PED_CAN_RAGDOLL((Ped)curPedPtr, false);
...
{
	if (curPedPtr != NULL) {
		ENTITY::DELETE_ENTITY((Entity*)&curPedPtr);
		//PED::REMOVE_PED_ELEGANTLY((Ped*)&curPedPtr); //crashes everything?
		curPedPtr = nullptr;
	}
	//delete curPedPtr;
}
void CPlayerPed::SetPosition(Vector3 vPos) {
	if (curPedPtr != NULL) {
		ENTITY::SET_ENTITY_COORDS((DWORD)curPedPtr, vPos.x, vPos.y, vPos.z, true, true, true, false);
	}
}
Vector3 CPlayerPed::GetRotation() {
	Vector3 myVec;
	myVec.x = ENTITY::GET_ENTITY_ROLL((Entity)curPedPtr);
...
{
	if (ped == NULL || !PED::IS_PED_IN_ANY_VEHICLE(ped, true)) return -3;
	UINT Vehicle = PED::GET_VEHICLE_PED_IS_USING(ped);
	if (VEHICLE::GET_PED_IN_VEHICLE_SEAT(Vehicle, -1) == ped) return (int)-1; //-1 = driver according to nativedb
	for (int i = 0; i < VEHICLE::GET_VEHICLE_MAX_NUMBER_OF_PASSENGERS(Vehicle); i++)
	{
		if (VEHICLE::GET_PED_IN_VEHICLE_SEAT(Vehicle, i) == ped)
			return i;
	}
	return -3;
}
...
bool CPlayerPed::IsAPassenger() {
	return GetPedSeat((DWORD)curPedPtr) != -1 ? true : false;
}
void CPlayerPed::GetQuaternion(float * quat)
{
	//assert(quat);

	if (curPedPtr)
	{
		ENTITY::GET_ENTITY_QUATERNION((Entity)curPedPtr, &quat[0], &quat[1], &quat[2], &quat[3]);
	}
}
void CPlayerPed::SetQuaternion(float * quat)
{
	//assert(quat);

	if (curPedPtr)
	{
#if 0 //Completely broken, don't know how to fix yet TODO
		ENTITY::SET_ENTITY_QUATERNION((Entity)curPedPtr, quat[0], quat[1], quat[2], quat[3]);
#endif
	}
}
Vector3 CPlayerPed::GetCoordsFromVecFor(float dist)  {
	Vector3 Position;
	if (curPedPtr) {
		Vector3 forward = GetForwardVector();
		GetPosition(&Position);
		Position.x = Position.x + forward.x * dist;
		Position.y = Position.y + forward.y * dist;
		Position.z = Position.z + forward.z * dist;
	}
	return Position;
}
Vector3 CPlayerPed::GetCoordsFromVecForUsingVec(Vector3 m_VecFor, float dist)  {
	Vector3 Position;
	if (curPedPtr) {
		Vector3 forward = m_VecFor;
		GetPosition(&Position);
		Position.x = Position.x + forward.x * dist;
		Position.y = Position.y + forward.y * dist;
		Position.z = Position.z + forward.z * dist;
...
BOOL CPlayerPed::SetArmedWeapon(UINT32 weaponHash) {
	if (curPedPtr) {
		WEAPON::GIVE_WEAPON_TO_PED((Ped)curPedPtr, weaponHash, -1, true, true);
		return true;
	}
	return false;
}
void CPlayerPed::SetPedSpeed(Entity ent, Vector3 Speed) { 
	if (curPedPtr) {
		//ENTITY::APPLY_FORCE_TO_ENTITY((Entity)curPedPtr, 4, Speed.x, Speed.y, Speed.z, 0.0, 0.0, 0.0, 0, true, true, true, true, true);
		ENTITY::SET_ENTITY_VELOCITY(ent, Speed.x, Speed.y, Speed.z);
	}
}
int	CPlayerPed::GetMoveState() {
	if (curPedPtr) {
		if (AI::IS_PED_SPRINTING((Ped)curPedPtr)) {
			return 1; //Sprinting
		} else {
			return 0; //Walking
		}
