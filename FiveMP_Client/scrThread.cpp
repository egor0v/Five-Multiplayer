...
#ifdef HOOKS_ENABLED
static hook::thiscall_stub<eThreadState(GtaThread*, uint32_t)> gtaThreadTick([]()
{
...
eThreadState WRAPPER GtaThread::Tick(uint32_t opsToExecute)
{
...
eThreadState Run(GtaThread* This)
{
	...
	if (activeThread->m_ctx.State != ThreadStateKilled)
	{
		This->DoRun();
	}
	...
	return activeThread->m_ctx.State;
}
eThreadState GtaThread::Reset(uint32_t scriptHash, void* pArgs, uint32_t argCount)
{
	rage::scrThread* activeThread = rage::scrEngine::GetActiveThread();

	memset(&m_ctx, 0, sizeof(m_ctx));

	activeThread->m_ctx.State = ThreadStateIdle;
	activeThread->m_ctx.ScriptHash = scriptHash;
	activeThread->m_ctx._mUnk1 = -1;
	activeThread->m_ctx._mUnk2 = -1;

	activeThread->m_ctx._set1 = 1;
