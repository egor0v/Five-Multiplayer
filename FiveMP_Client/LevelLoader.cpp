...
public:
	...
	virtual eThreadState Run() override {
		return (eThreadState)1;
	}

	virtual eThreadState Tick() {
		return (eThreadState)1;
	}

	virtual eThreadState Tick(UINT32 opsToExecute) override {
		return (eThreadState)1;
	}
...
#if 0 //Crashes everything under the regular game
...
{
 //Anything that deals with mounting, crashes when loading the game (probably already mounted?)
